...
import time
from scalespacegan.util.misc import (
    save_image,
    create_dir,
    print_same_line,
    parse_comma_separated_number_list,
)
from scalespacegan import dnnlib
from glob import glob


TARGET_IMG = np.asarray(Image.open(r"./000210.png"))
GAME_OUTPUT = "wuxiangzhilei/samples"
...
@click.option(
    "--output_dir",
    help="Location were the data will be stored",
    metavar="DIR",
    required=True,
)
...
# Optional
@click.option(
    "--n_processes", help="Number of subsequent processes", metavar="INT", default=12
)  # uses multiprocessing to download patches in parallel if greater than 1
@click.option(
    "--batch_size",
    help="Size of the multiprocessing batch",
    metavar="INT",
    default=1000,
)  # uses multiprocessing to download patches in parallel if greater than 1
...
@click.option(
    "--scales",
    help="What scales should be sampled",
    metavar="[NAME|A,B,C|none]",
    type=parse_comma_separated_number_list,
)
@click.option(
    "--samples",
    help="Sets number of samples per scale",
    metavar="[NAME|A,B,C|none]",
    type=parse_comma_separated_number_list,
)
...
def main(**kwargs):
    ...
    browser_res = 1024
    data_dir = os.path.join(root_dir, GAME_OUTPUT)
    output_res = (256, 256)
    darkness_thres = 0.1

    # Set number of sampels per scale
    selected_scales = [0, 1, 2, 3, 4, 5]
    sample_counts = [50, 150, 400, 3000, 3000, 3000]
    ...
    dst_dir_name = (
        dst_dir_name_base
        + "_"
        + str(total_sample_count)
        + "_"
        + str(output_res[0])
        + "_"
        + str(selected_scales[0])
        + "-"
        + str(selected_scales[-1] + 1)
    )
    ...
    for idx_scale, sample_count in zip(selected_scales, sample_counts):
        ...
        if n_processes <= 1:
            for idx_img, scale in items:
                ...
                print_same_line(
                    f"Processed {idx_img+1} out of {total_sample_count} images."
                )
        else:
            ...
            for i in range(0, sample_count, batch_size):
                ...
                while True:
                    ...
                    results = pool.starmap(
                        ...
                        [
                            ...
                            for (idx_img, scale) in batch_items
                        ],
                    )
                    failed_indices = []
                    for result in results:
                        if isinstance(result, Exception):
                            ...
                            failed_indices += [
                                int(s) for s in str(result).split() if s.isdigit()
                            ]
                    if len(failed_indices) > 0:
                        batch_items = [
                            items[i - curr_cummulative_samples] for i in failed_indices
                        ]
                ...
                print_same_line(
                    f"{np.clip(i+batch_size, a_min=0, a_max=sample_count)} patches out of {int(sample_count)} for scale {current_scale} finished"
                )
...
def process_item(
    ...
    img_space_scale = 720.0 / 2.0**scale
    # print("img_space_scale", img_space_scale)

    np.random.seed(idx_img)

    # specifies the region that should be captured
    min_pos = np.array([0.0, 0.0])
    max_pos = np.array([720.0, 720.0]) - img_space_scale

    sample = np.random.uniform(size=2)
    pos = sample * (max_pos - min_pos) + min_pos

    x_min = int(pos[0])
    x_max = int(pos[0] + img_space_scale)
    y_min = int(pos[1])
    y_max = int(pos[1] + img_space_scale)
    img = TARGET_IMG[y_min : y_max + 1, x_min : x_max + 1, :]
    ...
    #     ):  # kills the task if criteria are not fulfilled within 30 seconds
    #         wait = False
    #         ready = False
    #     if (
    #         "resetting" in messages_str
    #     ):  # if this message is present in logs image is almost loaded (might change with the upadtes of the viewer API)
    #         time.sleep(2)
    #         wait = False
    #         ready = True
